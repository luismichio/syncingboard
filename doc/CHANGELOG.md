...
- **Sync-all no longer multiplies by selection size** — with "Also update all board copies" on, selecting N copies of the same frame re-swept the board once per selected copy and queued every copy N times (e.g. 2 selected x 9 copies = 18 update calls instead of 9), blowing past the 10/min Miro image-update window. The copy sweep now runs once per unique (platform, fileKey, nodeId) — matching FigJam's entries-dedupe model.
...
- **`<VideoTabs>` single-video mode** — when only one of `figma`/`penpot` is provided, the component renders that video directly without the Figma/Penpot tab bar; used for the new "SyncingBoard showcase" hero video on `/docs/features` (placed before Getting Started). URL normalized to the site's `youtube-nocookie` privacy pattern.
