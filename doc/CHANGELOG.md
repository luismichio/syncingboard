...
- **Live Figma selection greyed out on Community plans**: the toggle now sits behind the account plan — unknown plan or a free/Community Figma tier disables the checkbox with a "COMMUNITY" chip and a hint (polling burns budget the Community rate limits do not cover). A persisted "I'm on a paid Figma plan — enable" link in the same card unblocks it for Pro accounts; the plan is re-detected from X-Figma-Tier headers whenever Figma reports one.
